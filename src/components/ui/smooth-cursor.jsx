...
const ProfessionalCursor = ({ isHovering }) => {
  return (
    <motion.svg
...
export function SmoothCursor({ springConfig = defaultSpringConfig, type = "professional" }) {
  ...
  useEffect(() => {
    ...
    const smoothMouseMove = (e) => {
      ...
      if (type === "professional" && speed > 0.1) {
        const currentAngle =
  ...
  return (
    <motion.div
      style={{
        ...
        translateX: "-50%",
        translateY: "-50%",
        ...(type === "professional" ? { rotate: rotation, scale } : {}),
        zIndex: 9999,
        pointerEvents: "none",
    ...
    >
      {type === "minimal" ? (
        <MinimalCursor isHovering={isHovering} />
      ) : (
