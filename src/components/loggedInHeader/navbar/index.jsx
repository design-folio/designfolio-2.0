...
export default function Navbar() {
  ...
  const navContent = (
    <div className="px-4 h-full flex items-center justify-between gap-4 md:gap-8">
      {/* Logo + mode switcher */}
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          <SheetTrigger asChild>
            <Button
              variant="secondary"
              size="icon"
              className="bg-[#F5F5F5] hover:bg-[#E8E8E8] dark:bg-[#3A3531] dark:hover:bg-[#4A4540] border border-black/10 dark:border-white/10 text-[#1A1A1A] dark:text-[#F0EDE7] h-10 w-10 rounded-full"
            >
              <Menu size={18} />
            </Button>
          </SheetTrigger>
          <SheetContent
            side="bottom"
            className="rounded-t-3xl bg-white dark:bg-[#2A2520] border-t border-black/10 dark:border-white/10 p-6 flex flex-col gap-4"
          >
            <VisuallyHidden>
            ...
            <div className="flex flex-col gap-1">
              <Button
                variant="ghost"
                className="w-full justify-start h-11 px-4 text-[14px] text-[#7A736C] dark:text-[#9E9893] hover:text-[#1A1A1A] dark:hover:text-[#F0EDE7] hover:bg-[#F5F5F5] dark:hover:bg-[#3A3531]"
            ...
            <div className="flex w-full items-center justify-between gap-3 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-black/10 dark:border-white/10">
              <div
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 rounded-xl text-[#7A736C] dark:text-[#9E9893] hover:bg-black/5 dark:hover:bg-white/5 hover:text-[#1A1A1A] dark:hover:text-[#F0EDE7]"
                  onClick={handleCopy}
                >
                  {isCopied ? (
                    <Check className="h-4 w-4 text-[#1A1A1A] dark:text-[#F0EDE7]" />
                  ) : (
      ...
      <div className="hidden md:flex items-center gap-2">
        <Tooltip delayDuration={100}>
          ...
          <TooltipContent side="bottom" className="bg-[#1A1A1A] dark:bg-[#F0EDE7] text-[#F0EDE7] dark:text-[#1A1A1A] text-xs px-2 py-1 rounded">
            Insights
          </TooltipContent>
        </Tooltip>
  ...
  if (isMacOS) {
    return (
      <TooltipProvider>
        <div
          className="fixed top-0 left-0 bg-white dark:bg-[#2A2520] border-b border-black/[0.08] dark:border-white/10 h-[62px]"
          style={{
