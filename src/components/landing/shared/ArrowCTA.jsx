...
export default function ArrowCTA({ label = "Get started for Free", size = "lg", href = "/signup" }) {
  ...
  return (
    <Link href={href} className="group inline-flex cursor-pointer items-center gap-0 rounded-full no-underline">
      <span
        className={`rounded-full bg-[--lp-text] px-6 font-medium text-[--lp-fg-white] transition-colors duration-500 ease-in-out group-hover:bg-[--lp-accent-hover] group-hover:text-white whitespace-nowrap ${textCls}`}
        style={{ paddingLeft: isLg ? "24px" : "16px", paddingRight: isLg ? "24px" : "16px" }}
      >
        {label}
      </span>
      <div
        className={`relative flex-shrink-0 overflow-hidden rounded-full bg-[--lp-text] text-[--lp-fg-white] transition-colors duration-500 ease-in-out group-hover:bg-[--lp-accent-hover] group-hover:text-white ${circleSz}`}
      >
        <ArrowUpRight
