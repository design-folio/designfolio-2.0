...
const TiptapEditor = ({ projectDetails, userDetails, classNames }) => {
  ...
  const editor = useEditor({
    ...
    content:
      projectDetails?.tiptapContent && Object.keys(projectDetails.tiptapContent).length > 0
        ...
        _updateProject(projectId, {
          ...
          if (userDetails) {
            const updatedProjects = userDetails?.projects?.map((item) =>
              item._id === projectId ? { ...item, tiptapContent: json, contentVersion: 2 } : item
            );
            ...
            // Note: To avoid event spam used lastSavedRef to check the difference in the content
            // Track edit event
            if (JSON.stringify(lastSavedContentRef.current) !== JSON.stringify(json)) {
  ...
  useEffect(() => {
    if (editor && projectDetails?.tiptapContent) {
      ...
      if (isEditorEmpty && Object.keys(projectDetails.tiptapContent).length > 0) {
  ...
  return (
    <div className={cn("project-editor-container", classNames.container)} ref={editorContainerRef}>
      ...
      <TiptapMenuBar editor={editor} showToolbar={showToolbar} onImageUpload={handleImageUpload} />
    </div>
  );
};
