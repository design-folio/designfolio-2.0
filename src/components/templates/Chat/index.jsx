...
export default function Chat({ isEditing = false, preview = false }) {
  ...
  // Map static step numbers to dynamic ones based on sectionOrder
  ...
  // Build reveal sequence once, skipping hidden sections in preview mode
  ...
  // Viewport-aware reveal: only advance steps while container is visible
  useEffect(() => {
    setChatRevealStep(0);
    let queueIndex = 0;
    let timerId = null;
    ...
    const scheduleNext = () => {
      ...
      timerId = setTimeout(() => {
        timerId = null;
        if (!isInViewRef.current) return; // paused — will resume on intersection
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        // Resume scheduling if we just entered the viewport and no timer is pending
        if (
        ...
        <AnimatePresence mode="popLayout">
          {chatRevealStep >= s(6) && !(preview && tools.length === 0) && (
            ...
            >
              ...
              <div className="w-8 h-8 shrink-0 mt-auto flex items-end">
                {chatRevealStep < getNextLeftStep("tools") && (
                  <motion.div
                    layoutId="matt-avatar-sequence"
                    className="w-8 h-8 rounded-full overflow-hidden border border-black/5 dark:border-white/5"
                  >
              ...
              <>
                {experiences.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="w-8 h-8 shrink-0 mt-auto flex items-end">
                      {chatRevealStep < getNextLeftStep("works") && (
                        <motion.div
                          layoutId="matt-avatar-sequence"
                          className="w-8 h-8 rounded-full overflow-hidden border border-black/5 dark:border-white/5"
                        >
          ...
          {chatRevealStep >= s(17) && !(preview && !hasAboutContent) && (
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="bg-white dark:bg-[#2A2520] p-3 sm:p-4 rounded-2xl rounded-tl-sm rounded-bl-sm transition-colors duration-700 border border-black/5 dark:border-white/5 w-fit">
                  {chatRevealStep === s(17) ? (
        ...
        <AnimatePresence mode="popLayout">
          {chatRevealStep >= s(18) && !(preview && !hasAboutContent) && (
            ...
            >
              ...
              <div className="w-8 h-8 shrink-0 mt-auto flex items-end">
                {chatRevealStep < getNextLeftStep("about") && (
                  <motion.div
                    layoutId="matt-avatar-sequence"
                    className="w-8 h-8 rounded-full overflow-hidden border border-black/5 dark:border-white/5"
                  >
              ...
              {hasAboutDescription ? (
                <div className="bg-white dark:bg-[#2A2520] px-4 py-3 rounded-2xl rounded-tl-sm rounded-bl-sm text-[#1A1A1A] dark:text-[#F0EDE7] text-[15px] leading-relaxed transition-colors duration-700 border border-black/5 dark:border-white/5">
                  <p>
                ...
                <div className="bg-white dark:bg-[#2A2520] px-4 py-3 rounded-2xl rounded-tl-sm rounded-bl-sm transition-colors duration-700 border border-black/5 dark:border-white/5">
                  <button
                    onClick={() => openSidebar?.(sidebars.about)}
