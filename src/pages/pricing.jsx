  ...
  return (
    ...
    >
      <p data-testid="text-intro">
        Choose the plan that works best for you. Get Pro access with all
        features included.
      </p>
          ...
          {PRO_PLANS.map((p, idx) => (
            ...
            >
              <div className="flex items-center gap-2 h-4">
                ...
                {p.save && (
                  <span className="text-[10px] font-bold text-[--lp-accent]">
                    Save {p.save}
                  </span>
                )}
              </div>
          ...
          <table className="w-full text-[14px]">
            <thead>
              <tr className="border-b border-[--lp-border] bg-[--lp-bg]">
                ...
                <th className="text-center py-3 px-4 font-semibold text-[--lp-text]" data-testid="header-pro">
                  Pro
                </th>
              </tr>
            </thead>
            <tbody>
              {comparisonRows.map((row, i) => (
                ...
                >
                  <td className="py-3 px-4 font-medium text-[--lp-text] align-top" data-testid={`feature-${row.testId}`}>
                    {row.feature}
                  </td>
                  <td className="py-3 px-4 text-[--lp-text-muted] align-top" data-testid={`free-${row.testId}`}>
                    <Cell value={row.free} />
                  </td>
                  <td className="py-3 px-4 text-[--lp-text] align-top" data-testid={`pro-${row.testId}`}>
      ...
      <Accordion type="single" collapsible className="w-full !space-y-0 -mt-2">
        <AccordionItem value="item-1" className="border-[--lp-border]" data-testid="accordion-faq-plans">
          <AccordionTrigger className="text-[15px] text-[--lp-text] hover:no-underline" data-testid="text-faq-plans">
          ...
          <AccordionContent className="text-[--lp-text-muted]" data-testid="text-faq-plans-answer">
            The Pro features are identical on all three plans. Longer billing
            periods simply cost less per month — you save around 30% on Quarterly
            and 50% on Yearly compared to paying monthly.
          </AccordionContent>
        </AccordionItem>

        <AccordionItem value="item-2" className="border-[--lp-border]" data-testid="accordion-faq-cancel">
          <AccordionTrigger className="text-[15px] text-[--lp-text] hover:no-underline" data-testid="text-faq-cancel">
            Can I cancel anytime?
          </AccordionTrigger>
          <AccordionContent className="text-[--lp-text-muted]" data-testid="text-faq-cancel-answer">
            Yes. You keep access to Pro until the end of your current billing
            cycle. No questions asked.
          </AccordionContent>
        </AccordionItem>

        <AccordionItem value="item-3" className="border-[--lp-border]" data-testid="accordion-faq-downgrade">
          <AccordionTrigger className="text-[15px] text-[--lp-text] hover:no-underline" data-testid="text-faq-downgrade">
            What happens to my portfolio if I downgrade to Free?
          </AccordionTrigger>
          <AccordionContent className="text-[--lp-text-muted]" data-testid="text-faq-downgrade-answer">
            Your portfolio stays live. You'll be limited to 2 case studies; extras
            stay hidden until you upgrade again. Custom domain support is also
            removed.
          </AccordionContent>
        </AccordionItem>

        <AccordionItem value="item-4" className="border-[--lp-border]" data-testid="accordion-faq-ai">
          <AccordionTrigger className="text-[15px] text-[--lp-text] hover:no-underline" data-testid="text-faq-ai">
            Which plans include the AI job tools?
          </AccordionTrigger>
          <AccordionContent className="text-[--lp-text-muted]" data-testid="text-faq-ai-answer">
            AI job matching, tailored resumes, cover letters, and mock interviews
            are all included with Pro — available on Monthly, Quarterly, and
            Yearly billing.
          </AccordionContent>
        </AccordionItem>

        <AccordionItem value="item-5" className="border-[--lp-border]" data-testid="accordion-faq-trial">
          <AccordionTrigger className="text-[15px] text-[--lp-text] hover:no-underline" data-testid="text-faq-trial">
            Is there a free trial?
          </AccordionTrigger>
          <AccordionContent className="text-[--lp-text-muted]" data-testid="text-faq-trial-answer">
            No free trial right now, but the Free plan lets you explore the
            portfolio builder with up to 2 case studies before you decide to
            upgrade.
          </AccordionContent>
        </AccordionItem>

        <AccordionItem value="item-6" className="border-[--lp-border]" data-testid="accordion-faq-payment">
        ...
        <AccordionItem value="item-7" className="border-[--lp-border]" data-testid="accordion-faq-refund">
